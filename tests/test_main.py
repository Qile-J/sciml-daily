...
def test_run_end_to_end_offline(tmp_path, monkeypatch):
    ...
    def gen(instr, msg):
        return '[{"id":1,"in_scope":true,"tags":["operator-learning"],"summary":"r"}]'
    ...
    # SPA: index.html is a static shell; the paper ships in data.json, rendered client-side.
    assert (tmp_path / "docs" / "index.html").exists()
    assert (tmp_path / "docs" / "data.json").read_text().count("Neural operator") >= 1
